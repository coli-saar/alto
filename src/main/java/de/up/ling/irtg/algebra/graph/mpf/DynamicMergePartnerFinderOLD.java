 ...
 * @author jonas
 */
public class DynamicMergePartnerFinderOLD extends MergePartnerFinder {
    ...
    public DynamicMergePartnerFinderOLD(int currentSource, int nrSources, int nrNodes, SGraphBRDecompositionAutomatonBottomUp auto)//maybe give expected size of finalSet as parameter?
    {
        this.auto = auto;
        /*if (nrSources == 0){
         //isFinal = true;
    ...
    public DynamicMergePartnerFinderOLD(int currentSource, int nrSources, int nrNodes, SGraphBRDecompositionAutomatonBottomUp auto, boolean hasAll, IntSet vertices)//maybe give expected size of finalSet as parameter?
    {
        this.auto = auto;
        /*if (nrSources == 0){
         //isFinal = true;
    ...
    private void insertInto(int index, int rep, IntSet newVertices) {
        if (children[index] != null) {
            ...
            if (sourcesRemaining == 1) {
                ...
                children[index] = new DynamicMergePartnerFinderOLD(sourceNr + 1, sourcesRemaining - 1, children.length - 2, auto, (hasAll || (index == ALL)), newVertices);
            }
            children[index].insert(rep);
        }
    }
