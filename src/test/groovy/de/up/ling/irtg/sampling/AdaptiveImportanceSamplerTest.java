 ...
 * @author teichmann
 */
public class AdaptiveImportanceSamplerTest {
    /**
     *
     */
     ...
     * 
     */
    private AdaptiveImportanceSampler adaSamp;
    ...
    public void setUp() {
        ...
        adaSamp = new AdaptiveImportanceSampler(9883498483484L);
        ...
        for(Rule r : (Iterable<Rule>) this.tau.getRulesTopDown(startState)) {
            switch (r.toString(tau)) {
                ...
                case "'0-6'! -> *('0-3', '3-6') [1.0]":
                    ...
                    inside = auw.getAutomaton().inside();
                    d = inside.get(r.getChildren()[0]);
                    d += inside.get(r.getChildren()[1]);
                    assertEquals(d / inside.get(r.getParent()),Math.exp(auw.getLogProbability(r)),0.05);
                    break;
            }
